...
def loading_data(filepath):
    ...
    dfCrashes['CRASH_DATE'] = pd.to_datetime(dfCrashes['CRASH_DATE'])  # [DA1]
    dfCrashes['Month'] = dfCrashes['CRASH_DATE'].dt.month  # [DA7]
    dfCrashes = dfCrashes.dropna(subset=['LAT', 'LON'])  # [GPT]
    return dfCrashes


def lineplot(dfCrashes, show_grid=True):  # [PY1], [VIZ1]
    MonthlyCrashes = dfCrashes.groupby('Month').size().reset_index(name='Crashes')      # [DA2]  [GPT]
...
def piechart(data, column, title):  # [VIZ3]
    ...
    explode = [0.1 if value < threshold else 0 for value in countData]  # [PY4] [GPT]
...
# For this map, I utilized a YouTube video by Curiosity Data Analytics for help with the map
# https://www.youtube.com/watch?v=gBU8DgLfu5Q
